...
const ShopTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-popover border border-border rounded-lg p-2 shadow-xl text-xs">
      ...
      <p className="font-medium mt-1">Total: {d.total.toLocaleString()} tCO2e</p>
    </div>
  );
};

const BreakdownPanels = ({ onShopClick, onScopeClick, activeScope }: BreakdownPanelsProps) => {
  const scopeTotals = [
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div className="bg-card rounded-lg border border-border p-5">
        ...
        <p className="text-xs text-muted-foreground mb-3">tCO2e — click to filter</p>
      ...
      <div className="bg-card rounded-lg border border-border p-5">
        ...
        <p className="text-xs text-muted-foreground mb-3">tCO2e — click to filter categories</p>
        <ResponsiveContainer width="100%" height={240}>
          <PieChart>
            <Pie
              data={scopeTotals}
          ...
          {filteredCategories.slice(0, 5).map((cat) => (
            <div key={cat.category} className="group cursor-pointer">
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <span className="text-muted-foreground font-mono">{cat.value.toLocaleString()} tCO2e</span>
              </div>
